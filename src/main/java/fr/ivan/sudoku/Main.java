...
public class Main {
    ...
    static void testGridForSudoku(int size, String grid, Sudoku sdk) {
        System.out.println(new Date());
        timeIt(() -> sdk.SetGrid(grid, size));
        timeIt(sdk::Solve);
        sdk.PrintGrid();
    }

    static void testGrid(int size, String grid){
        testGridForSudoku(size, grid, new Procedural());
        testGridForSudoku(size, grid, new Backtracking());
    }
